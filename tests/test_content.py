...
def test_time_ContentMixed(inputs, testdata_collaborative, testdata_content, limit = 100):
   ...
   # s to ms
    ...
    recommendation, strategy, error = mixed_strategy.get_recommendations(
        bike_id, family_id, price, frame_size_code, n)

    end_time = time.time()
    assert end_time - \
        start_time < limit, f"ContentMixed took {(end_time - start_time)*1000} ms, limit is {limit*1000} ms"


def test_len_sample_(inputs, testdata_collaborative, testdata_content, n_test=100):
    ...
    for i in data_store_content.similarity_matrix.sample(n_test).index:
        bike_id = i
        ...
        assert len(
            recommendation) == n, f"ContentMixed recommendation has {len(recommendation)} rows for bike_id {bike_id}, which is not {n} rows"
...
def test_time_ContentMixed(inputs, testdata_collaborative, testdata_content, limit=100):
    ...
    recommendation, strategy, error = mixed_strategy.get_recommendations(bike_id, family_id, price, frame_size_code, n)
...
def test_len_sample_(inputs, testdata_collaborative, testdata_content, n_test=100):
    ...
    rows = list(data_store_content.similarity_matrix.rows)
    sampled_rows = random.sample(rows, n_test)
    for i in sampled_rows:
        bike_id = i

        mixed_strategy = ContentMixed(logger, data_store_collaborative, data_store_content)
        strategy, recommendation, error = mixed_strategy.get_recommendations(bike_id, family_id, price, frame_size_code, n)
