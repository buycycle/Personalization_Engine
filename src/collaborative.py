...
def get_top_n_collaborative(model, user_id: str, n: int, dataset, df_status_masked, logger) -> Tuple[List, Optional[str]]:
    ...
    try:
        ...
        top_item_ids = [item_index_id_map[item_id]
...
def get_top_n_collaborative_randomized(model, user_id: str, n: int, sample: int, dataset, df_status_masked, logger) -> Tuple[List, Optional[str]]:
    """
    Retrieve the top k item ids for a given user_id by using model.predict()
...
def create_data_model_collaborative(DB,
                                    driver,
                                    query,
                                    user_id,
                                    bike_id,
